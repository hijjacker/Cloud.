...
with tab_explanation:
    ...
    st.write("""
            ...
            I've fiddled around with Streamlit once, but never published anything, so this was a new experience for me. I only little practical experience with coding outside of CS106B, but the website itself was a super fun/manageable challenge. The trickiest part was definitely incorporating the GTM model, which became a little more technical than I expected.
             """)
...
with tab_classifier:
    ...
    st.write("I used Google Teachable Machine to train this model, which is a relatively easy way to go about it. It only required images to actually train the model (exporting it was a little trickier!). I trained it on five different classes, cirrus, cirrocumulus, cumulus, cumulonimbus, and stratus. I uploaded ~50 images of each, a small number of which were my own, but most came from the web. After testing it and adjusting a few parameters, I tried to export it. Usually, to use it in a webapp you just copy paste the JavaScript that GTM provides. However, I made this website using Streamlit, so I had to figure out how to make it compatabile with my Python script. I first tried exporting it using a Keras file. I tried to incorporate this into my code, but ran into threading issues, as Python3.13 isn't compatabile with tensorflow. This took me an embarrassingly long time to figure out because I couldn't understand the error message. I ended up using a TensorflowLite file which only runs inference on the model from the cloud (doesn't actually host it), which is slightly less accurate but somehow solved the threading issue. And to my surprise it worked! Definitely the most difficult part of the project, but the most worthwhile!")
